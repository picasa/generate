...
layout_sequence <- function(
    seq, map, cut = 80, flow = 0, scale = c(2.5, 5), noise = c(0, 0), orientation = "lrtb") {
  ...
  layout <- string |>
    ...
    dplyr::mutate(e_c = ifelse(
      ...
      - dplyr::first(offset), stats::rnorm(dplyr::n(), 0, noise[1])), .by = row) |>
...
layout_paragraph <- function(
    seq, map, cut = 80, flow = 0, scale = c(2.5, 5), noise = c(0, 0), spacing = 1, orientation = "lrtb") {
